...
/**
 * Week day names in distict languages.
 */
var DAYS = {
...
/**
 * Months names in distict languages.
 */
var MONTHS = {
...
/**
 * Cache of collectors to avoid instantiating them over and over again
 * @type {Object} Object including a property per collector specification, this is:
 *                {
 *                  "module.exports = [[...]];": collector1,
 *                  ...
 *                  "module.exports = [[...]];": collectorN
 *                }
 */
var collectors = {};
...
function checkPendingInvocations() {
  for (var ii = 0; ii < updateJobs.length; ii++) {
    if (updateJobs[ii] && updateJobs[ii].pendingInvocations().length === 0) {
 ...
 */
function getUpdateRequestOptions(elementType, element, attributes) {
  /* jshint camelcase: false */
...
/**
 * Returns a collector object for a concrete specification
 * @param  {String} spec The specification
 * @return {Object}      The collector object
 */
function getCollector(spec) {
  if (!collectors[spec]) {
    collectors[spec] = fiwareDeviceSimulatorCollector(spec);
  }
  return collectors[spec];
}

/**
 * Sends a request for external data.
 * @param {Object}  The request options
 * @param {Function} The callback
 */
function getExternalData(options, callback) {
  request(options, callback);
}

/**
 * Generates the final external URL in case it uses some date templates.
 * @param  {String} url The external url.
 * @return {String}     The final external url with all the date templates subtituted by their final values.
 */
function generateExternalURL(url) {
  var now = new Date();
  var finalURL = url.replace(/{YY}/g, now.getFullYear().toString().substring(2,4));
  finalURL = finalURL.replace(/{YYYY}/g, now.getFullYear().toString());
  var currentMonth011 = now.getMonth(),
      currentMonth112 = currentMonth011 + 1;
  
  finalURL = finalURL.replace(/{MM}/g, currentMonth112 < 10 ? '0' + currentMonth112 : currentMonth112.toString());
  finalURL = finalURL.replace(/{M\*}@en/g, MONTHS.en[currentMonth011].toUpperCase());
  finalURL = finalURL.replace(/{m\*}@en/g, MONTHS.en[currentMonth011].toLowerCase());
  finalURL = finalURL.replace(/{MMM}@en/g, MONTHS.en[currentMonth011].substring(0,3).toUpperCase());
  finalURL = finalURL.replace(/{mmm}@en/g, MONTHS.en[currentMonth011].substring(0,3).toLowerCase());
  finalURL = finalURL.replace(/{M\*}@es/g, MONTHS.es[currentMonth011].toUpperCase());
  finalURL = finalURL.replace(/{m\*}@es/g, MONTHS.es[currentMonth011].toLowerCase());
  finalURL = finalURL.replace(/{MMM}@es/g, MONTHS.es[currentMonth011].substring(0,3).toUpperCase());
  finalURL = finalURL.replace(/{mmm}@es/g, MONTHS.es[currentMonth011].substring(0,3).toLowerCase());
  var currentDate = now.getDate();
  finalURL = finalURL.replace(/{DD}/g, currentDate < 10 ? '0' + currentDate : currentDate.toString());
  finalURL = finalURL.replace(/{DD-1}/g, currentDate < 10 ? '0' + (currentDate - 1) : (currentDate - 1).toString());
  var currentDay = now.getDay();
  finalURL = finalURL.replace(/{D\*}@en/g, DAYS.en[currentDay].toUpperCase());
  finalURL = finalURL.replace(/{D\*}@en/g, DAYS.en[currentDay].toLowerCase());
  finalURL = finalURL.replace(/{DDD}@en/g, DAYS.en[currentDay].substring(0, 3).toUpperCase());
  finalURL = finalURL.replace(/{ddd}@en/g, DAYS.en[currentDay].substring(0, 3).toLowerCase());
  finalURL = finalURL.replace(/{D\*}@es/g, DAYS.es[currentDay].toUpperCase());
  finalURL = finalURL.replace(/{D\*}@es/g, DAYS.es[currentDay].toLowerCase());
  finalURL = finalURL.replace(/{DDD}@es/g, DAYS.es[currentDay].substring(0, 3).toUpperCase());
  finalURL = finalURL.replace(/{ddd}@es/g, DAYS.es[currentDay].substring(0, 3).toLowerCase());
  var currentHours = now.getHours();
  finalURL = finalURL.replace(/{hh}/g, currentHours < 10 ? '0' + currentHours : currentHours.toString());
  var currentMinutes = now.getMinutes();
  finalURL = finalURL.replace(/{mm}/g, currentMinutes < 10 ? '0' + currentMinutes : currentMinutes.toString());
  var currentSeconds = now.getSeconds();
  finalURL = finalURL.replace(/{ss}/g, currentSeconds < 10 ? '0' + currentSeconds : currentSeconds.toString());
  return finalURL;
}
...
function update(elementType, element, attributes) {
  ...
  if (elementType === 'entity' || (elementType === 'device' &&
      (element.protocol === 'UltraLight::HTTP' || element.protocol === 'JSON::HTTP'))) {
    if (elementType === 'entity' && element.external) {
      var externalRequestOptions = {
        method: element.external.method,
        url: generateExternalURL(element.external.url),
        headers: element.external.headers,
        json: element.external.json,
        body: element.external.body
      };
      async.retry(element.external.retry || { times: 1 }, async.apply(getExternalData, externalRequestOptions),
        function(err, response, body) {
          emitResponse(err, externalRequestOptions, body, response);
          if(err) {
            updatesResponded++;
            process.nextTick(checkPendingInvocations);
            nextTick();
            ...
            if (element.external.collector) {
              element.external.response = response;
              collection = getCollector(element.external.collector).collect(element.external);
            }
            async.eachSeries(
              collection,
              function(item, callback) {
                var finalAttributes = item.concat(attributes);
                var requestOptions = getUpdateRequestOptions(elementType, element, finalAttributes);
                if (requestOptions) {
      ...
      emitRequest(externalRequestOptions);
      updatesRequested++;
    } else {
      var requestOptions = getUpdateRequestOptions(elementType, element, attributes);
      if (requestOptions) {
        request(requestOptions, function(err, response, body) {
          emitResponse(err, requestOptions, body, response);
          updatesResponded++;
