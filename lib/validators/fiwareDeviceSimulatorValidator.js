...
var urlValidator = require('check-valid-url');
...
/**
 * Validates external elements
 * @param  {String}   external    The external element to validate
 * @param  {String}   parentType  The associated parent element type
 * @param  {Number}   parentIndex The associated parent element index
 * @param  {Function} callback    The callback
 */
function validateExternal(external, parentType, parentIndex, callback) {
  if (external) {
    if (typeof external !== 'object') {
      return callback(new fdsErrors.SimulationConfigurationNotValid('The ' + parentType +
            ' configuration information at array index position ' + parentIndex +
            ' includes an invalid \'external\' property' +
            ' (it should be an object): \'' +
            JSON.stringify(external) + '\''));
    } else {
      if (external.retry) {
        if (typeof external.retry !== 'object') {
          return callback(new fdsErrors.SimulationConfigurationNotValid('The ' + parentType +
              ' configuration information at array index position ' + parentIndex +
              ' includes an invalid \'external\' property' +
              ' (the optional \'retry\' subproperty is not an object): \'' +
              JSON.stringify(external) + '\''));
        } else {
          if (external.retry.times && typeof external.retry.times !== 'number') {
            return callback(new fdsErrors.SimulationConfigurationNotValid('The ' + parentType +
              ' configuration information at array index position ' + parentIndex +
              ' includes an invalid \'external\' property' +
              ' (the optional \'retry\' subproperty does not contain a valid \'times\' subproperty, it should be' +
              ' a number): \'' +
              JSON.stringify(external) + '\''));
          }
          if (external.retry.interval && typeof external.retry.interval !== 'number') {
            return callback(new fdsErrors.SimulationConfigurationNotValid('The ' + parentType +
              ' configuration information at array index position ' + parentIndex +
              ' includes an invalid \'external\' property' +
              ' (the optional \'retry\' subproperty does not contain a valid \'interval\' subproperty, it should be' +
              ' a number): \'' +
              JSON.stringify(external) + '\''));
          }
        }
      ...
      if (external.method) {
        if (typeof external.method !== 'string') {
          return callback(new fdsErrors.SimulationConfigurationNotValid('The ' + parentType +
            ' configuration information at array index position ' + parentIndex +
            ' includes an invalid \'external\' property' +
            ' (the mandatory \'method\' subproperty is not an string): \'' + JSON.stringify(external) + '\''));
        } else {
          if (['GET', 'POST'].indexOf(external.method) === -1) {
            return callback(new fdsErrors.SimulationConfigurationNotValid('The ' + parentType +
              ' configuration information at array index position ' + parentIndex +
              ' includes an invalid \'external\' property' +
              ' (the mandatory \'method\' subproperty is not equal to \'GET\' or \'POST\'): \'' +
              JSON.stringify(external) + '\''));
          }
        }
      } else {
        return callback(new fdsErrors.SimulationConfigurationNotValid('The ' + parentType +
          ' configuration information at array index position ' + parentIndex +
          ' includes an invalid \'external\' property' +
          ' (the mandatory \'method\' subproperty is missing): \'' + JSON.stringify(external) + '\''));
      }
      if (!external.url) {
        return callback(new fdsErrors.SimulationConfigurationNotValid('The ' + parentType +
          ' configuration information at array index position ' + parentIndex +
          ' includes an invalid \'external\' property' +
          ' (the mandatory \'url\' subproperty is missing): \'' + JSON.stringify(external) + '\''));
      } else if (!urlValidator.isUrl(external.url)) {
        return callback(new fdsErrors.SimulationConfigurationNotValid('The ' + parentType +
          ' configuration information at array index position ' + parentIndex +
          ' includes an invalid \'external\' property' +
          ' (the mandatory \'url\' subproperty does not seem to be a valid URL): \'' + JSON.stringify(external) + '\''));
      }
      if (external.headers && typeof external.headers !== 'object') {
        return callback(new fdsErrors.SimulationConfigurationNotValid('The ' + parentType +
          ' configuration information at array index position ' + parentIndex +
          ' includes an invalid \'external\' property' +
          ' (the optional \'headers\' subproperty is not an object): \'' + JSON.stringify(external) + '\''));
      }
      if (external.json && typeof external.json !== 'boolean') {
        return callback(new fdsErrors.SimulationConfigurationNotValid('The ' + parentType +
          ' configuration information at array index position ' + parentIndex +
          ' includes an invalid \'external\' property' +
          ' (the optional \'json\' subproperty is not an boolean): \'' + JSON.stringify(external) + '\''));
      }
      if (external.body && typeof external.body !== 'object') {
        return callback(new fdsErrors.SimulationConfigurationNotValid('The ' + parentType +
          ' configuration information at array index position ' + parentIndex +
          ' includes an invalid \'external\' property' +
          ' (the optional \'body\' subproperty is not an object): \'' + JSON.stringify(external) + '\''));
      }
      if (!external.collector) {
        return callback(new fdsErrors.SimulationConfigurationNotValid('The ' + parentType +
          ' configuration information at array index position ' + parentIndex +
          ' includes an invalid \'external\' property' +
          ' (the \'collector\' subproperty is missing): \'' + JSON.stringify(external) + '\''));
      } else if (typeof external.collector !== 'string') {
        return callback(new fdsErrors.SimulationConfigurationNotValid('The ' + parentType +
          ' configuration information at array index position ' + parentIndex +
          ' includes an invalid \'external\' property' +
          ' (the \'collector\' subproperty is not a string): \'' + JSON.stringify(external) + '\''));
      }
    }
  }
  setImmediate(callback);
...
function validateEntityConfiguration(entityConfiguration, index, callback) {
  ...
  if (!entityConfiguration.external &&
    (!entityConfiguration.staticAttributes || entityConfiguration.staticAttributes.length === 0) &&
    (!entityConfiguration.active || entityConfiguration.active.length === 0)) {
    return callback(new fdsErrors.SimulationConfigurationNotValid('The entities configuration information ' +
      'at array index position ' + index + ' misses either external or static and/or active attributes configuration ' +
      'information'));
  }
  async.series([
    async.apply(validateSchedule, entityConfiguration.schedule, 'entity', index),
    async.apply(validateExternal, entityConfiguration.external, 'entity', index),
